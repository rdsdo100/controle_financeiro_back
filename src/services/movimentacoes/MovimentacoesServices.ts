import { IBuscaMovimentacoes } from "../../controller/movimentacoes/MovimentacoesController";
...
export default class MovimentacoesServices {
    /*
    ...
    */
    ...
    async buscarMovimentacoesFiltro({ idUsuario, busca, tipoBusca, pagina }: IBuscaMovimentacoes) {
        ...
        const retornoBusca = await movimentacoesRepository.buscarMovimentacoesBusca({
            idUsuario,
            busca,
            tipoBusca,
            pagina,
    ...
    async deleteMovimentacao({ usuarioId, movimentacoesId }: IDeleteMovimentacos) {
        const movimentacoesRepository = new MovimentacoesRepository();
        const movimentacoes: Movimentacoes = await movimentacoesRepository.buscarMovimentacoesUserId(
            usuarioId,
            movimentacoesId,
        );
        ...
        if (movimentacoes.tipoPoupanca) {
            movimentacoes.contasIdFK.poupanca =
                Number(movimentacoes.contasIdFK.poupanca) - Number(movimentacoes.valorMovimento);
        } else {
            movimentacoes.contasIdFK.corrente =
                Number(movimentacoes.contasIdFK.corrente) - Number(movimentacoes.valorMovimento);
        }
        movimentacoes.contasIdFK.valorTotal =
            Number(movimentacoes.contasIdFK.corrente) + Number(movimentacoes.contasIdFK.poupanca);
        const retorno: string = await movimentacoesRepository.deleteMovimetacoesReposiroty(
            movimentacoes.id,
            movimentacoes.contasIdFK,
        );
    ...
    async registerMovimentacaoConta(movimentacoes: Movimentacoes) {
        ...
        if (!saveMovimentacoes.tipoPoupanca) {
            saveMovimentacoes.contasIdFK.corrente =
                Number(saveMovimentacoes.contasIdFK.corrente) + Number(saveMovimentacoes.valorMovimento);
        } else {
            saveMovimentacoes.contasIdFK.poupanca =
                Number(saveMovimentacoes.contasIdFK.poupanca) + Number(saveMovimentacoes.valorMovimento);
        }

        saveMovimentacoes.contasIdFK.valorTotal =
            Number(saveMovimentacoes.contasIdFK.corrente) + Number(saveMovimentacoes.contasIdFK.poupanca);
